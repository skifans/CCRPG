...
import enemies as e #Saves time by importing arrays instead of copy-pasting them into code.
...
ename = None #So that ename is defined globally
...
def terminate():
    #time.sleep(2) #Might be added in later
    pygame.quit()
    sys.exit()
...
def type_select(player_class):
    ...
    if player_class == "barbarian":
            ...
            player = ninja
            image = pygame.image.load(os.path.join("textures","necromancer.png"))
    #Changed all class images to necromancer for testing
    else:
...
def classselect(classes,lancer,archer,necromancer,warrior,mage,paladin,barbarian,samurai,ninja):
    ...
    try:
        ...
        debug("Failed to load class texture, ending")
        terminate()
    player[7]=name
    mana = player[3]*2
    mana_use = mana
...
def levelupcheck():
    ...
    return
# the enemy arrays
enemy1 = [e.darkness.magnus, #Gets the variables from enemies module
e.air.air_wizard,
e.earth.earth_wizard,
e.earth.tree,
e.fire.fire_bat,
e.fire.fire_wizard,
e.heaven.priest,
...
def enemyturn ():
    ...
    if echoice == 1:
        ...
        if ehit < 0:
            if enemyhit > 0:
                php = php - enemyhit
                print(enemy + " hits you for "+ str(enemyhit))
                if php <= 0:
                    print("You were killed by " + enemy)
                    combatover = True
            if enemyhit <= 0:
                print(enemy + " does no damage")
        else:
            print(enemy + " misses")
    else:
        print(enemy + " tries to cast a spell!")
        print("It fails!")
#-------------------------------------------------------------------------------
def spell_image_blue_puff():
    try:
        ...
        debug("Spell image not found, terminating")
        time.sleep(2)
        pygame.quit()
        sys.exit(1)
